                 ...
                 const BpConnectionClass & provider)
{
    if(producer.GetRouteType() != provider.GetRouteType()) {
        BpInterconnect::ErrorMsg("Cannot compare connections when route type is different");
    };
                                               ...
                                               const BpConnectionClass & provider)
{
    if (producer.GetRouteType() != provider.GetRouteType()) 
    {
        ErrorMsg("Mismatching ML connection. The 1st argument is '%s' and the 2nd argument is '%s'\n", ((producer.GetRouteType() == TLM1) ? "TLM1" : "TLM2"), ((provider.GetRouteType() == TLM1) ? "TLM1" : "TLM2"));
        return false;
    }
	...
	// OK
        ...
        ErrorMsg("Mismatching ML connection. The 1st argument is '%s' and the 2nd argument is '%s'\n", (producer.GetIntfName() + " interface in " + producer.GetPath()).c_str(), (provider.GetIntfName() +  " interface in " + provider.GetPath()).c_str());
        return false;
      }
    }
    ...
    {
        ErrorMsg("Incorrect order of arguments for UVM ML connection function. Arguments are %s and %s\n", producer.GetPath().c_str(), provider.GetPath().c_str());
        return false;
    } 
    else
    {
                                                      ...
                                                      int producerBindKey)
{
    ...
    if(producer == NULL) {
      ErrorMsg("Cannot identify producer by ID %d", producerBindKey);
    }
    assert (producer != NULL); // Internal error

    BpConnectionClass * provider = producer->GetSingleProvider();
    if(producer == NULL) {
      ErrorMsg("Cannot identify unique provider for producer");
    }
                                                      ...
                                                      int providerBindKey)
{
    ...
    if(provider == NULL) {
      ErrorMsg("Cannot identify provider by ID %d", providerBindKey);
    }
    assert (provider != NULL); // Internal error

    BpConnectionClass * producer = provider->GetSingleProducer();
    if(provider == NULL) {
      ErrorMsg("Cannot identify unique producer for provider");
    }
                           ...
                           double           timeValue)
{
    ...
    if(producer == NULL) {
      ErrorMsg("Cannot identify unique producer by ID %d", producerBindKey);
    }
    assert (producer != NULL); // Internal error
...
void BpInterconnect::TurnOffTransactionMapping(int UNUSED frameworkId, const string & socketName)
{
