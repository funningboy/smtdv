...
UvmMlPrintService*       CommonServicesFramework::m_print_service        = 0;
    ...
    FrameworkProxyClass(-1, frameworkName, frameworkIndicators, requiredApi)
{
    ...
    srv_provider.wakeup_srv_provider = "";
    srv_provider.stopper_srv_provider = "";
    srv_provider.print_srv_provider = GetName().c_str();
...
int CommonServicesFramework::phase_srv_check_phase(
    const char * phaseName)
{
    return m_phase_service->srvc_check_phase(phaseName);
}

int CommonServicesFramework::phase_srv_check_future_phase(
    const char * phaseName,
    uvm_ml_phase_action phaseAction)
{
  return 1;
  //return m_phase_service->srvc_check_future_phase(phaseName, phaseAction);
}

void CommonServicesFramework::print_srv_print_request(const char * message)
{
    m_print_service->srvc_print_request(message);
}
///////////////////////////////////////////////////////////////////////
...
bp_frmw_c_api_struct * CommonServicesFramework::getRequiredApi()
{
    ...
    requiredApi->phase_srv_check_phase_ptr       = CommonServicesFramework::phase_srv_check_phase;
    requiredApi->print_srv_print_request_ptr     = CommonServicesFramework::print_srv_print_request;
