...
/* Delay-based callback registration and notification API */
/***********************************************/
//! typedef for bp_register_time_cb function ptr
/*! Register time-based callback
 *
 *  @param framework_id               - Unique ID of the framework adapter
 *  @param time_unit                  - Time unit, according to the uvm_ml_time_unit enum
 *  @param time_value                 - Desired simulation time, in time units as defined by the time_unit parameter
 *  @param cb_data                    - Optional farmework-specific data to be passed back when the callback is triggered
 *
 *  @return Unique callback handle
 *           - non-null : success
 *           - 0 : error
 */
typedef uvm_ml_handle (*bp_register_time_cb_type)
(
    int               framework_id,
    uvm_ml_time_unit  time_unit,
    double            time_value,
...
//! typedef for bp_remove_time_cb function ptr
/*! Cancel previously registered time-based callback
 *
 *  @param framework_id         - Unique ID of the framework adapter
 ...
 *           - 0 : error
 */
typedef int (*bp_remove_time_cb_type)
(
    int           framework_id,
    uvm_ml_handle cb_handle
);

/*! Notify the appropriate framework of a time-based callback
 *
 *  @param framework_id               - Unique ID of the framework adapter
 *  @param time_unit                  - Time unit, according to the uvm_ml_time_unit enum
 *  @param time_value                 - The simulation time at which the callback was triggered, in time units as defined by the time_unit parameter
 *  @param cb_data                    - Framework-specifc data that was passed in at the callback registration
 *
 *  @return Return status
 *           - 1 : success
 *           - 0 : error
...
/* ML Events API */
/***********************************************/
typedef void (*bp_notify_event_type)
(
    int                           framework_id,
...
typedef void (*bp_notify_time_cb_type)
(
    int              framework_id,
...
/* ML Barriers API */
/***********************************************/
typedef void (*bp_notify_barrier_type)
(
    int                           framework_id,
    const char *                  scope_name,
...
/*! Request message to be printed by the registered print service provider
 *
 *  @param message                  - Message to print
 */
typedef void (*bp_print_request_type)
(
    const char * message
);


/*! Execute generic UVM-ML command
 *
 *  @param framework_id       - Unique ID of the framework adapter (optional - relevant only for the commands ariginated by some adapter)
 *  @param command            - Full command string
 *  @param output_cb          - Callback function pointer (optional) - if the caller wants to get the output rather than the regustered print service
 *  @param time_unit          - Time unit, according to the uvm_ml_time_unit enum (optional, can be set to TIME_UNIT_UNDEFINED to specify that it's N/A (for example, in interactive commands)
 *  @param time_value         - The simulation time at which the callback was triggered, in time units as defined by the time_unit parameter
 *
 *  @return Return status
 *           - 1 : success
 ...
 *
 */
typedef int (*bp_execute_command_type)
(
    int              framework_id,
    const char *     command,
...
// ----- ML Hierarchical construction API
   ...
   */
    ...
    /*! Register time-based callback request
     *
     *  \param framework_id               - Unique ID of the framework adapter
     *  \param time_unit                  - Time unit, according to the uvm_ml_time_unit enum
     *  \param time_value                 - Value of the time, in time units as defined by the time_unit parameter
     *  \param cb_data                    - Data to be passed back when the callback is triggered
     *
     *  \return int
     *           - 1 : success
     *           - 0 : error
    ...
    /*! Notify backplane of an event creation or update
     *
     * \param framework_id - unique id of the caller framework
     * \param scope_name - full hierarchical name of a pool
     * \param event_name - name of an event
     * \param aciton - ID of the action
     * \stream_size - size of the message stream
     * \stream - the message stream; may represent a packed typed object
     * \time_unit - time units
     * \time_value - current time
     * \return void
    */
    ...
    /*! Notify backplane of a barrier creation or update
     * \param framework_id - unique id of the caller framework
     * \param scope_name - full hierarchical name of a pool
     * \param barrier_name - name of a barrier
     * \param aciton - ID of the action
     * \count - current barrier counter
     * \time_unit - time units
     * \time_value - current time
     * \return void
    */
...
//------------------------------------------------------------------------------
//! Process the relevant simulator command line arguments (Incisive-only)
/*
 *
 */

extern int uvm_ml_bp_elaborate(void *cbInfo);
