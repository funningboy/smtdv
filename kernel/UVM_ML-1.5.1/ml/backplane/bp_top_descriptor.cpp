...
void BpTopDescriptorBaseClass::SetFrameworkTopLevelComponent()
{
    ...
    const string & topName = frmwProxy->GetTopComponentForArg(GetIdentifier());
    if (!topName.empty()) 
      {
	  ...
	  //Temporay, till we support type and instance name as valid parameter
	    ...
	    SetInstanceName(topName);
          ...
          // FIXME - comment out unused variable
	  //BpTopDescriptorClass * topComponent
	  frmwProxy->AddTopLevel(this);
      }
}
