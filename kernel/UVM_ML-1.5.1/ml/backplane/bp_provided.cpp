...
bp_api_struct * uvm_ml_bp_bootstrap( char **uvmTops,
                              char **uvmTests,
                             bp_serror_t   *   serror) {
    vector<string> tops;
    ...
    return Bp::BpInterconnect::Bootstrap(tops,tests, serror);
}


int uvm_ml_bp_elaborate(void *cbInfo){ 
    return BpInterconnect::Elaborate(cbInfo);
}
 ...
 *              
 */
static void bp_set_trace_mode(int mode)
{
 ...
 * 
 */
static int bp_register_framework(char *                 framework_name,
 ...
 * 
 */
static char * bp_get_version() 
{
  char * result = const_cast<char *> (BpInterconnect::GetVersion().c_str());
  BpInterconnect::DebugTrace("In bp_get_version(). Returned version is %s\n", result);
  return result;
}
 ...
 * 
 */
static int bp_add_root_node (unsigned     adapter_id, 
 ...
 * 
 */
static int bp_run_test(int framework_id, int tops_n, char ** tops, char * test)
{

    BpInterconnect::DebugTrace("In bp_run_test() called by framework %d\n", framework_id);

    Bp::BpInterconnect::PreRunCleanup(); // Allow multiple tests activated back-to-back
...
//------------------------------------------------------------------------------
//! React to the simulator reset event
/*
 *
 */
static void bp_reset()
{
    BpInterconnect::Reset();
}

//------------------------------------------------------------------------------
//! Return the state of the trace mode flag
/*
 * This function is to be invoked directly rather than via the 'provided' tray
 * in order to enable trace printouts as early as the adapter registration stage
 */

int bp_interconnect_get_trace_mode() {
    return Bp::BpInterconnect::GetTraceMode();
}  
 ...
 * 
 */
static void bp_synchronize(int framework_id, uvm_ml_time_unit time_unit, double time_value)
{
 ...
 * 
 */
static unsigned bp_connect(unsigned       framework_id, 
                    const char *   path1,
                    const char *   path2)
{
    BpInterconnect::DebugTrace("In bp_connect() called by framework %d for producer_path = %s provider_path = %s\n", framework_id, path1, path2);
    unsigned result = BpInterconnect::RegisterRoute(framework_id, path1, path2);
    BpInterconnect::DebugTrace("In bp_connect() result = %d\n", result);
    return result;
}
 ...
 * 
 */
static int bp_get_connector_size(int framework_id, int connector_id) 
{
  int result = BpInterconnect::GetConnectorSize(framework_id,connector_id); 
  BpInterconnect::DebugTrace("In bp_get_connector_size() for framework %d with connector_id %d. Result is: %d\n",
 ...
 * 
 */
static void bp_notify_end_blocking(int              framework_id,
                            int              callback_framework_id, 
                            int              call_id,
                            uvm_ml_time_unit time_unit,
                            double           time_value) 
{ 
    BpInterconnect::DebugTrace("In bp_notify_end_blocking() called by framework %d\n", framework_id);
 ...
 * 
 */
static int bp_nb_put(int              framework_id,
              int              producer_id,
              ...
              double           time_value)
{
    BpInterconnect::DebugTrace("In bp_nb_put()\n");
 ...
 * 
 */
static int bp_can_put(int              framework_id, 
	       int              producer_id,
	       uvm_ml_time_unit time_unit,
	       double           time_value) 
{ 
    BpInterconnect::DebugTrace("In bp_can_put()\n");
 ...
 * 
 */
static int bp_put(int                framework_id,
           int                producer_id,
           unsigned int       stream_size,
           uvm_ml_stream_t    stream,
           uvm_ml_time_unit * time_unit,
 ...
 * 
 */
static int bp_request_put(int                framework_id,
                   int                producer_id,
                   int                call_id, 
                   unsigned int       stream_size, 
                   uvm_ml_stream_t    stream,
                   ...
                   double           * time_value) 
{
    ...
    BpInterconnect::DebugTrace("In bp_request_put()\n");
    bool done_tmp = false;
 ...
 * 
 */
static int bp_nb_get(int               framework_id, 
              int               producer_id,
              ...
              double            time_value) 
{ 
    BpInterconnect::DebugTrace("In bp_nb_get()\n");
 ...
 * 
 */
static int bp_can_get(int              framework_id, 
               int              producer_id,
               uvm_ml_time_unit time_unit,
               double           time_value) 
{ 
    BpInterconnect::DebugTrace("In bp_can_get()\n");
 ...
 */
static int bp_get(int                framework_id,
           int                producer_id,
           uvm_ml_stream_t    stream,
           uvm_ml_time_unit * time_unit,
           double           * time_value) 
{ 
    BpInterconnect::ErrorMsg("Get not implemented");
    assert (false);
    return 0;
}
 ...
 * 
 */
static int bp_request_get(int                framework_id,
                   int                producer_id,
                   int                call_id,
                   unsigned int *     stream_size_ptr,
                   uvm_ml_stream_t    stream,
                   ...
                   double           * time_value) 
{ 
    BpInterconnect::DebugTrace("In bp_request_get()\n");
    bool done_tmp = false;
 ...
 * 
 */
static unsigned int bp_get_requested(int             framework_id,
			      int             producer_id,
			      int             call_id,
			      uvm_ml_stream_t stream)
{ 
    BpInterconnect::DebugTrace("In bp_get_requested()\n");
 ...
 * 
 */
static int bp_nb_transport(int               framework_id, 
                    int               producer_id, 
                    ...
                    double            time_value)
{ 
    BpInterconnect::DebugTrace("In bp_nb_transport()\n");
 ...
 * 
 */
static int bp_transport(int                framework_id, 
                 int                producer_id,
                 unsigned int       req_stream_size, 
                 uvm_ml_stream_t    req_stream,
                 ...
                 double           * time_value)
{ 
    BpInterconnect::ErrorMsg("Transport not implemented");
    assert (false);
    return 0;
}
 ...
 *          - 1 - if the corresponding thread was disabled before normal completion
 */
static int bp_request_transport(int                framework_id, 
                         int                producer_id,
                         int                call_id,
                         unsigned int       req_stream_size, 
                         uvm_ml_stream_t    req_stream,
                         ...
                         double           * time_value) 
{ 
    BpInterconnect::DebugTrace("In bp_request_transport()\n");
    bool done_tmp = false;
 ...
 *  @return Output stream size.
 */
static unsigned int bp_transport_response(int              framework_id, 
                                   int              producer_id,
                                   int              call_id,
                                   uvm_ml_stream_t  rsp_stream) 
{ 
    BpInterconnect::DebugTrace("In bp_transport_response()\n");
 ...
 * 
 */
static void bp_write(int              framework_id, 
              int              producer_id, 
              unsigned int     stream_size, 
              uvm_ml_stream_t  stream,
              uvm_ml_time_unit time_unit,
              double           time_value)
{ 
    BpInterconnect::DebugTrace("In bp_write()\n");
 ...
 *  @return Value returned by the export
 */
static int bp_nb_peek(int              framework_id,
               int              producer_id,
               unsigned int *   stream_size_ptr,
               uvm_ml_stream_t  stream,
               uvm_ml_time_unit time_unit,
               double           time_value) 
{
    BpInterconnect::DebugTrace("In bp_nb_peek()\n");
 ...
 *  @return Value returned by the export
 */
static int bp_can_peek(int              framework_id,
                int              producer_id,  
                uvm_ml_time_unit time_unit,
                double           time_value) 
{
    BpInterconnect::DebugTrace("In bp_can_peek()\n");
 ...
 *  @return Returns 0 - unsupported.
 */
static unsigned int bp_peek(int                framework_id,
                     int                producer_id,
                     uvm_ml_stream_t    stream,
                     uvm_ml_time_unit * time_unit,
                     double           * time_value)
{
    BpInterconnect::ErrorMsg("Peek not implemented");
    assert(false);
    return 0;
}
 ...
 *          - 1 - if the corresponding thread was disabled before normal completion
 */
static int bp_request_peek(int                framework_id, 
                    int                producer_id,
                    int                call_id,
                    unsigned int     * stream_size_ptr,
                    uvm_ml_stream_t    stream,
                    ...
                    double           * time_value) 
{ 
    BpInterconnect::DebugTrace("In bp_request_peek()\n");
    bool done_tmp = false;
 ...
 *  @return Output stream size.
 */
static unsigned int bp_peek_requested(int              framework_id,
                               int              producer_id,
                               int              call_id, 
                               uvm_ml_stream_t  stream)
{
    BpInterconnect::DebugTrace("In bp_peek_requested()\n");
 ...
 *  @return Returns 0, unsupported.
 */
static int bp_b_transport_tlm2(int                framework_id, 
                        int                initiator_id, 
                        unsigned int     * stream_size,
                        uvm_ml_stream_t  * stream,
...
{
    BpInterconnect::ErrorMsg("bp_b_transport_tlm2 not implemented");
    assert (false);
    return 0;
}
 ...
 *          - 1 - if the corresponding thread was disabled before normal completion
 */
static int bp_request_b_transport_tlm2(int               framework_id, 
                                int               initiator_id, 
                                int               call_id,
                                unsigned int *    stream_size,
                                uvm_ml_stream_t * stream,
                                ...
                                double            time_value)
{
  BpInterconnect::DebugTrace("In bp_request_b_transport_tlm2()\n");
 ...
 */
static int bp_b_transport_tlm2_response(int             framework_id, 
                                 int             initiator_id, 
                                 int             call_id,
                                 unsigned int *  stream_size,
                                 uvm_ml_stream_t stream)
{
  BpInterconnect::DebugTrace("In bp_b_transport_tlm2_response()\n");
 ...
 */
static uvm_ml_tlm_sync_enum bp_nb_transport_fw(int                framework_id, 
                                        int                initiator_id,
                                        unsigned int *     stream_size,
                                        uvm_ml_stream_t *  stream,
                                        ...
                                        double             time_value)
{
  BpInterconnect::DebugTrace("In bp_nb_transport_fw()\n");
 ...
 */
static uvm_ml_tlm_sync_enum bp_nb_transport_bw(int                framework_id, 
                                        int                target_id,
                                        unsigned int *     stream_size,
                                        uvm_ml_stream_t *  stream,
                                        uvm_ml_tlm_phase * phase,
                                        ...
                                        double             time_value)
{
  BpInterconnect::DebugTrace("In bp_nb_transport_bw()\n");
 ...
 *  @return Value returned by the target
 */
static unsigned int bp_transport_dbg(int               framework_id, 
                              int               initiator_id,
                              unsigned int *    stream_size,
                              uvm_ml_stream_t * stream, 
                              uvm_ml_time_unit  time_unit, 
                              double            time_value)
{
  BpInterconnect::DebugTrace("In bp_transport_dbg()\n");
 ...
 */
static unsigned int bp_assign_transaction_id(int framework_id)
{
  BpInterconnect::DebugTrace("In bp_assign_transaction_id()\n");
 ...
 */
static void bp_turn_off_transaction_mapping(int framework_id, const char *socket_name)
{
  BpInterconnect::DebugTrace("In bp_turn_off_transaction_mapping() called by framework %d.\n", framework_id);
 ...
 *  @return  <0 : error
 */
static int bp_create_child_junction_node(int            framework_id,
...
{
  BpInterconnect::DebugTrace("In bp_create_child_junction_node()\n");
 ...
 */
static uvm_ml_type_id_t bp_get_type_id_from_name(int framework_id, const char* type_name) 
{ 
    uvm_ml_type_id_t result = BpInterconnect::GetTypeIdByTypeName(framework_id,string(type_name));
    BpInterconnect::DebugTrace("In bp_get_type_id_from_name() called by framework %d for type name %s. Returned id is %d\n", framework_id, type_name, (int) result);
    return result;
}
 ...
 */
static char* bp_get_type_name_from_id(int framework_id, uvm_ml_type_id_t type_id) 
{ 
    char * result = const_cast<char *> (BpInterconnect::GetTypeNameByTypeId(framework_id,type_id,true).c_str());
    BpInterconnect::DebugTrace("In bp_get_type_name_from_id() called by framework %d for type id %d. Returned type name is %s\n", framework_id, type_id, result);
    return result;
}
 ...
 *           - 0 : error
 */
static int bp_set_match_types(int framework_id, const char* type1, const char* type2) {
    BpInterconnect::DebugTrace("In bp_set_match_types() called by framework %d with type1 = %s type2 = %s\n",framework_id,type1,type2);
...
int bp_get_pack_max_size(int framework_id) {
    BpInterconnect::DebugTrace("In bp_get_pack_max_size()\n");
 ...
 *  @return void
 */
static void bp_set_pack_max_size(int framework_id,int size)
{
    BpInterconnect::DebugTrace("In bp_set_pack_max_size() called by framework %d. New size is %d\n", framework_id, size);
 ...
 */
static void bp_register_srv_providers (
    unsigned int            framework_id,
    bp_srv_provider_struct *srv_providers)
{
    BpInterconnect::DebugTrace("In bp_register_srv_provider()\n");
    ...
    uvm_ml_phase_action phase_action)
{
    ...
    BpInterconnect::DebugTrace("In bp_notify_phase() '%s'\n", phase_name);
    ...
    uvm_ml_phase_action phase_action)
{
    ...
    BpInterconnect::DebugTrace("In bp_transmit_phase()\n");
 ...
 *           - 0 : error
 */
static void bp_notify_runtime_phase (
    unsigned int        framework_id,
    const char *        phase_group,
    const char *        phase_name,
    uvm_ml_phase_action phase_action,
    ...
    unsigned int *      participate)
{
    ...
    BpInterconnect::DebugTrace("In bp_notify_runtime_phase()\n");
 ...
 *  @param time_value    - Current simulatin time scaled according to time_unit
 */
static void bp_notify_phase_done (
    unsigned int      framework_id,
    const char *      phase_group,
    const char *      phase_name,
    uvm_ml_time_unit  time_unit,
    double            time_value)
{
    BpInterconnect::DebugTrace("In bp_notify_phase_done()\n");
...
static int bp_notify_resource (
    ...
    double                        time_value)
{
    BpInterconnect::DebugTrace("In bp_notify_resource() with\n");
    BpInterconnect::DebugTrace("            item_scope = %s item_name = %s\n", item_scope, item_name);
    BpInterconnect::DebugTrace("            action = %d stream_size = %d\n", action, stream_size);
    BpInterconnect::DebugTrace("            stream kind = %d accessor_name = %s\n", *(int *)stream, accessor_name);
    BpInterconnect::DebugTrace("\n");
...
static void bp_notify_config (
    ...
    double           time_value)
{
    BpInterconnect::DebugTrace("In bp_notify_config() with \n");
    BpInterconnect::DebugTrace("            context = %s instance_name = %s field_name = %s\n", cntxt, instance_name, field_name);
    BpInterconnect::DebugTrace("            stream_size = %d \n", stream_size);
    BpInterconnect::DebugTrace("            stream kind = %d\n", *(int *)stream);
    BpInterconnect::DebugTrace("\n");

    BpInterconnect::NotifyConfig(framework_id, cntxt, instance_name, field_name, stream_size, stream, time_unit, time_value);
}
///////////////////////////////////////////////////////////////////////
/// Register a request for time-based callback         t
/// At the given time, the framework specified by the ID, should be notified
/// @param framework_id  - Unique ID of the framework adapter
/// @param time_unit     - time unit
/// @param time_value    - the requested simulation time expressed in time units as
///                        given by the time_unit parameter
/// @cb_data             - optional data to be passed back when the callback is triggered
///////////////////////////////////////////////////////////////////////

static uvm_ml_handle bp_register_time_cb (
    int              framework_id,
    uvm_ml_time_unit time_unit,
    double           time_value,
    void *           cb_data)
{
    return BpInterconnect::RegisterTimeCallback(framework_id, time_unit, time_value, cb_data);
}
///////////////////////////////////////////////////////////////////////
/// Remove a request for time-based callback         t
/// @param cb_handle     - Unique callback handle returned by bp_register_time_cb
///////////////////////////////////////////////////////////////////////

static int bp_remove_time_cb (
    int           framework_id,
    uvm_ml_handle cb_handle
)
{
    return BpInterconnect::RemoveTimeCallback(framework_id, cb_handle);
}
///////////////////////////////////////////////////////////////////////
/// Notify the specified framework of a time-based callback
/// The framework must request the callback previously by invoking bp_register_time_cb
/// @param framework_id  - Unique ID of the framework adapter
/// @param time_unit     - time unit
/// @param time_value    - the simulation time at which the callback was triggered,
///                      - expressed in time units as given by the time_unit parameter
/// @param cb_data       - callback-related data passed in at registration
///////////////////////////////////////////////////////////////////////

static void bp_notify_time_cb (
    int              framework_id,
    int              target_framework_id,
    uvm_ml_time_unit time_unit,
    double           time_value,
    void *           cb_data)
{
    BpInterconnect::NotifyTimeCallback(framework_id, target_framework_id, time_unit, time_value, cb_data);

}

//////////////////////////////////////////////////////////////////////////////////////////
/// Request from the backplane to pass a message to the regsitered print service provider
/// @param message        - Message to be printed by the service provider
///////////////////////////////////////////////////////////////////////

static void bp_print_request (
    const char *        message
)
{
    BpInterconnect::PrintMessage(message);
}
...
int bp_execute_command (int framework_id, const char * command, bp_output_cb_t * output_cb, uvm_ml_time_unit time_unit, double time_value)
{
    return  BpInterconnect::BpDebugClass::ExecuteCommand(framework_id, command, output_cb, time_unit, time_value);
}

///////////////////////////////////////////////////////////////////////
/// Broadcast notification about an event
/// Indicates to the backplane that a pool name, event name,
/// action id and value should be transmitted to all involved frameworks
/// @param framework_id  - Unique ID of the framework adapter
/// @param scope_name - full hierarchical name of a pool
/// @param event_name - name of an event
/// @param action - ID of action
/// @param stream_size - size of the message stream
/// @param stream - the message stream; may represent a packed typed object
/// @param time_unit     - Current simulation time unit
/// @param time_value    - Current simulatin time scaled according to time_unit
///////////////////////////////////////////////////////////////////////
static void bp_notify_event (
    int                            framework_id,
    ...
    double                         time_value)
{
    BpInterconnect::DebugTrace("UVM-ML BP>> In bp_notify_event() with \n");
    BpInterconnect::DebugTrace("            scope_name = %s event_name = %s action = %d\n", scope_name, event_name, action);
    BpInterconnect::DebugTrace("            stream_size = %d \n", stream_size);
    BpInterconnect::DebugTrace("            stream kind = %d\n", *(int *)stream);
    BpInterconnect::DebugTrace("\n");

    BpInterconnect::NotifyEvent(framework_id, scope_name, event_name, action, stream_size, stream, time_unit, time_value);

}

///////////////////////////////////////////////////////////////////////
/// Broadcast notification about a barrier
/// Indicates to the backplane that a pool name, barrier name,
/// action id and value should be transmitted to all involved frameworks
/// @param framework_id  - Unique ID of the framework adapter
/// @param scope_name - full hierarchical name of a pool
/// @param barrier_name - name of a barrier
/// @param action - ID of action
/// @param count - count data
/// @param time_unit     - Current simulation time unit
/// @param time_value    - Current simulatin time scaled according to time_unit
///////////////////////////////////////////////////////////////////////
static void bp_notify_barrier (
    int                            framework_id,
    const char *                   scope_name,
    ...
    double                         time_value)
{
    BpInterconnect::DebugTrace("UVM-ML BP>> In bp_notify_barrier() with \n");
    BpInterconnect::DebugTrace("            scope_name = %s barrier_name = %s action = %d\n", scope_name, barrier_name, action);
    BpInterconnect::DebugTrace("            data = %d \n", count);
    BpInterconnect::DebugTrace("\n");

    BpInterconnect::NotifyBarrier(framework_id, scope_name, barrier_name, action, count, time_unit, time_value);
...
void BpInterconnect::fillCApiTray() 
{
  ...
  SET_TRAY_ITEM(register_time_cb);
  SET_TRAY_ITEM(remove_time_cb);
  SET_TRAY_ITEM(notify_time_cb);

  SET_TRAY_ITEM(print_request);


  SET_TRAY_ITEM(notify_event);
  SET_TRAY_ITEM(notify_barrier);
  SET_TRAY_ITEM(execute_command);
}
