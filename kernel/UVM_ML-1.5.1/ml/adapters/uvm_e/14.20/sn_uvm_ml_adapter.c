...
#define SN_UVM_ML_ADAPTER (GLOB_SIM->ml_lib_adapter)
#define SN_UVM_ML_WAKEUP_SRV (SN_UVM_ML_ADAPTER->srv_provider->wakeup_srv_provider)
#define CREATE_SN_UVM_ML_ADAPTER_IF_NEEDED				\
  do {									\
    if (!SN_UVM_ML_ADAPTER) 						\
...
/*                                */
void sn_register_service_providers(SN_TYPE(sn_uvm_ml_srv_provider_manager) srv_provider) {

  bp_srv_provider_struct provider;

  ENTER_DEBUG_MSG_VOID("sn_register_service_providers");
  
  provider.phase_srv_provider = srv_provider->phase_srv_provider->frmw_name;
  provider.wakeup_srv_provider = srv_provider->wakeup_srv_provider->frmw_name; 
  provider.print_srv_provider = "Specman";

  provided_tray->register_srv_providers_ptr(sn_uvm_ml_adapter_id,&provider);
  EXIT_DEBUG_MSG_VOID("sn_register_service_providers");
...
static int sn_bp_startup() 
{
  ...
  CREATE_SN_UVM_ML_ADAPTER_IF_NEEDED;
  ...
  while (result && (e_top = pop0_node_data())) {
    ...
    if (e_top[e_top_len-1] != 'e' || e_top[e_top_len-2] != '.') {
      ...
      SN_DISPATCH(sn_ml_out,SN_UVM_ML_ADAPTER,sn_ML_LIB_adapter,(SN_UVM_ML_ADAPTER,err_msg));
      result = 0;
    } else {
  ...
  if (result) { 
    ...
    SN_DISPATCH(sn_ml_init_phases,SN_UVM_ML_ADAPTER,sn_ML_LIB_adapter,(SN_UVM_ML_ADAPTER));
...
static void sn_bp_set_debug_mode(int mode)    
{
  ...
  if (GLOB && GLOB_SIM && SN_UVM_ML_ADAPTER) {
    SN_DISPATCH(sn_ml_set_debug_mode,SN_UVM_ML_ADAPTER,sn_ML_LIB_adapter,(SN_UVM_ML_ADAPTER,mode));
  }
...
static int sn_bp_find_connector_id_by_name (const char * path)
{
  ...
  result = SN_DISPATCH(sn_ml_find_connector_id_by_name,SN_UVM_ML_ADAPTER,sn_ML_LIB_adapter,(SN_UVM_ML_ADAPTER,path));
...
static int sn_bp_get_connector_type(unsigned connector_id)
{
  ...
  result = SN_DISPATCH(sn_ml_get_connector_type,SN_UVM_ML_ADAPTER,sn_ML_LIB_adapter,(SN_UVM_ML_ADAPTER,connector_id));
...
static unsigned sn_bp_is_export_connector(unsigned connector_id)
{
  ...
  result = SN_DISPATCH(sn_ml_is_export_connector,SN_UVM_ML_ADAPTER,sn_ML_LIB_adapter,(SN_UVM_ML_ADAPTER,connector_id));
...
static const char * sn_bp_get_connector_intf_name(unsigned connector_id)
{
  ...
  result = (char *)SN_DISPATCH(sn_ml_get_connector_intf_name,SN_UVM_ML_ADAPTER,sn_ML_LIB_adapter,(SN_UVM_ML_ADAPTER,connector_id));
...
static char* sn_bp_get_connector_T1_name(int port_id)
{
  ...
  result = (char *)SN_DISPATCH(sn_ml_get_connector_T1_name,SN_UVM_ML_ADAPTER,sn_ML_LIB_adapter,(SN_UVM_ML_ADAPTER,port_id));
...
static char* sn_bp_get_connector_T2_name(int port_id)
{
  ...
  result = (char *)SN_DISPATCH(sn_ml_get_connector_T2_name,SN_UVM_ML_ADAPTER,sn_ML_LIB_adapter,(SN_UVM_ML_ADAPTER,port_id));
				   ...
				   double time_value)
{
  ...
  result = SN_DISPATCH(sn_ml_try_put_bitstream,SN_UVM_ML_ADAPTER,sn_ML_LIB_adapter,(SN_UVM_ML_ADAPTER,connector_id,stream_size,stream));
			 ...
			 double time_value)
{
  ...
  result = SN_DISPATCH(sn_ml_can_put,SN_UVM_ML_ADAPTER,sn_ML_LIB_adapter,(SN_UVM_ML_ADAPTER,connector_id));
				   ...
				   double time_value)
{
  ...
  result = SN_DISPATCH(sn_ml_try_get_bitstream,SN_UVM_ML_ADAPTER,sn_ML_LIB_adapter,(SN_UVM_ML_ADAPTER,connector_id,stream_size_ptr,stream));
			 ...
			 double time_value)
{
  ...
  result = SN_DISPATCH(sn_ml_can_get,SN_UVM_ML_ADAPTER,sn_ML_LIB_adapter,(SN_UVM_ML_ADAPTER,connector_id));
				    ...
				    double time_value)
{
  ...
  ENTER_DEBUG_MSG("sn_bp_try_peek_bitstream","connector_id = %d, stream_size_ptr = %p, stream = %p, time_unit = %d, time_value = %f",
		  connector_id,stream_size_ptr,stream,time_unit,time_value);
  result = SN_DISPATCH(sn_ml_try_peek_bitstream,SN_UVM_ML_ADAPTER,sn_ML_LIB_adapter,(SN_UVM_ML_ADAPTER,connector_id,stream_size_ptr,stream));
			  ...
			  double time_value)
{
  ...
  result = SN_DISPATCH(sn_ml_can_peek,SN_UVM_ML_ADAPTER,sn_ML_LIB_adapter,(SN_UVM_ML_ADAPTER,connector_id));
...
{ 
  ...
  SN_DISPATCH(sn_ml_put_bitstream_request,SN_UVM_ML_ADAPTER,sn_ML_LIB_adapter,(SN_UVM_ML_ADAPTER,connector_id,call_id,callback_adapter_id,stream_size,stream));
				       ...
				       double time_value)
{
  ...
  SN_DISPATCH(sn_ml_get_bitstream_request,SN_UVM_ML_ADAPTER,sn_ML_LIB_adapter,(SN_UVM_ML_ADAPTER,connector_id,call_id,callback_adapter_id));
...
{  
  ...
  ENTER_DEBUG_MSG("sn_bp_peek_bitstream_request","connector_id = %d, call_id = %d, callback_adapter_id = %d, time_unit = %d, time_value = %f",
		  connector_id,call_id,callback_adapter_id,time_unit,time_value);
  SN_DISPATCH(sn_ml_peek_bitstream_request,SN_UVM_ML_ADAPTER,sn_ML_LIB_adapter,(SN_UVM_ML_ADAPTER,connector_id,call_id,callback_adapter_id));
...
{  
  ...
  SN_DISPATCH(sn_ml_transport_bitstream_request,SN_UVM_ML_ADAPTER,sn_ML_LIB_adapter,(SN_UVM_ML_ADAPTER,connector_id,call_id,callback_adapter_id,req_stream_size,req_stream));
					      ...
					      uvm_ml_stream_t stream) 
{
  ...
  result = SN_DISPATCH(sn_ml_get_requested_bitstream,SN_UVM_ML_ADAPTER,sn_ML_LIB_adapter,(SN_UVM_ML_ADAPTER,connector_id,call_id,stream));
					       ...
					       uvm_ml_stream_t stream) 
{
  ...
  result = SN_DISPATCH(sn_ml_peek_requested_bitstream,SN_UVM_ML_ADAPTER,sn_ML_LIB_adapter,(SN_UVM_ML_ADAPTER,connector_id,call_id,stream));
...
static unsigned sn_bp_transport_response_bitstream(unsigned connector_id, 
  ...
  result = SN_DISPATCH(sn_ml_get_requested_bitstream,SN_UVM_ML_ADAPTER,sn_ML_LIB_adapter,(SN_UVM_ML_ADAPTER,connector_id,call_id,rsp_stream));
...
static void sn_bp_turn_off_transaction_mapping(const char* socket_name) 
{
  ...
  SN_DISPATCH(sn_ml_turn_transaction_mapping_off,SN_UVM_ML_ADAPTER,sn_ML_LIB_adapter,(SN_UVM_ML_ADAPTER,socket_name));
				  ...
				  double time_value)
{
  ...
  SN_DISPATCH(sn_ml_notify_end_task,SN_UVM_ML_ADAPTER,sn_ML_LIB_adapter,(SN_UVM_ML_ADAPTER,call_id));
					...
					double time_value)
{
  ...
  result = SN_DISPATCH(sn_ml_nb_transport_bitstream,SN_UVM_ML_ADAPTER,sn_ML_LIB_adapter,(SN_UVM_ML_ADAPTER,connector_id,req_stream_size,req_stream,rsp_stream_size_ptr,rsp_stream));
				  ...
				  double time_value)
{
  ENTER_DEBUG_MSG("sn_bp_write_bitstream","connector_id = %d, stream_size= %d, stream = %p, time_unit = %d, time_value = %f",
		  connector_id,stream_size,stream,time_unit,time_value);
  SN_DISPATCH(sn_ml_write_bitstream,SN_UVM_ML_ADAPTER,sn_ML_LIB_adapter,(SN_UVM_ML_ADAPTER,connector_id,stream_size,stream));
					  ...
					  double time_value)
{
  ...
  result = SN_DISPATCH(sn_ml_tlm2_b_transport_request,SN_UVM_ML_ADAPTER,sn_ML_LIB_adapter,(SN_UVM_ML_ADAPTER,target_connector_id,call_id,callback_adapter_id,stream_size,stream,delay_unit,time));
					   ...
					   uvm_ml_stream_t *stream)
{
  ...
  result = SN_DISPATCH(sn_bp_tlm2_b_transport_response,SN_UVM_ML_ADAPTER,sn_ML_LIB_adapter,(SN_UVM_ML_ADAPTER,initiator_connector_id,call_id,stream_size,stream));
						       ...
						       double time_value)
{
  ...
  result = SN_DISPATCH(sn_ml_tlm2_nb_transport_fw,SN_UVM_ML_ADAPTER,sn_ML_LIB_adapter,(SN_UVM_ML_ADAPTER,target_connector_id,stream_size,stream,phase,transaction_id,delay_unit,&time));
						       ...
						       double time_value)
{
  ...
  ENTER_DEBUG_MSG("sn_bp_tlm2_nb_transport_bw","target_connector_id  = %d, stream_size_ptr = %p, stream = %p, phase_ptr = %p, transaction_id = %d, delay_unit_ptr = %p, delay_value_ptr = %p, time_unit = %d, time_value = %f",
		  target_connector_id,stream_size,stream,phase,transaction_id,delay_unit,delay_value,time_unit,time_value);
  result = SN_DISPATCH(sn_ml_tlm2_nb_transport_bw,SN_UVM_ML_ADAPTER,sn_ML_LIB_adapter,(SN_UVM_ML_ADAPTER,target_connector_id,stream_size,stream,phase,transaction_id,delay_unit,&time));
  *delay_value = SN_REAL_GET(time);
					 ...
					 double time_value)
{
  ...
  result = SN_DISPATCH(sn_ml_tlm2_transport_dbg,SN_UVM_ML_ADAPTER,sn_ML_LIB_adapter,(SN_UVM_ML_ADAPTER,target_connector_id,stream_size,stream));
					    ...
					    int          parent_junction_node_id) 
{
  ...
  result = SN_DISPATCH(sn_ml_create_child,SN_UVM_ML_ADAPTER,sn_ML_LIB_adapter,(SN_UVM_ML_ADAPTER,component_type_name_copy,instance_name_copy,parent_full_name_copy,parent_framework_id,parent_junction_node_id));
			      ...
			      uvm_ml_phase_action phase_action) 
{
  ...
  result = SN_DISPATCH(sn_uvm_ml_notify_phase_wrap,SN_UVM_ML_ADAPTER,sn_ML_LIB_adapter,(SN_UVM_ML_ADAPTER,phase_group,phase_name,phase_action));
				...
				uvm_ml_phase_action phase_action)
{
  ...
  result = SN_DISPATCH(sn_uvm_ml_transmit_phase_wrap,SN_UVM_ML_ADAPTER,sn_ML_LIB_adapter,(SN_UVM_ML_ADAPTER,child_component_id,phase_group,phase_name,phase_action));
				...
				double           time_value) 
{
  ...
  SN_DISPATCH(sn_ml_notify_config,SN_UVM_ML_ADAPTER,sn_ML_LIB_adapter,(SN_UVM_ML_ADAPTER,cntxt_copy,instance_name_copy,field_name_copy,stream_size,stream));

  EXIT_DEBUG_MSG_VOID("sn_bp_notify_config");
}

static uvm_ml_handle sn_bp_register_time_cb(int              target_framework_id,
				            uvm_ml_time_unit time_unit,
				            double           time_value,
				            void *           cb_data) 
{
  uvm_ml_handle result;
  SN_TYPE(real) sn_time_value;

  ENTER_DEBUG_MSG("sn_bp_register_time_cb","target_framework_id = %d, time_unit = %d, time_value = %f, cb_data = %p",
		  target_framework_id,time_unit,time_value,cb_data);

  sn_time_value = SN_REAL_NEW(time_value);

  result = (uvm_ml_handle) SN_DISPATCH(sn_ml_register_time_cb,SN_UVM_ML_WAKEUP_SRV,sn_uvm_ml_wakeup_srv_provider,(SN_UVM_ML_WAKEUP_SRV,target_framework_id,time_unit,sn_time_value,cb_data));
  
  EXIT_DEBUG_MSG("sn_bp_register_time_cb","result cb_handle = %ld",result);

  return result;
}

static int sn_bp_remove_time_cb(uvm_ml_handle cb_handle) 
{
  int result;

  ENTER_DEBUG_MSG("sn_bp_remove_time_cb","cb_handle = %ld",cb_handle);
  
  result = (int) SN_DISPATCH(sn_ml_remove_time_cb,SN_UVM_ML_WAKEUP_SRV,sn_uvm_ml_wakeup_srv_provider,(SN_UVM_ML_WAKEUP_SRV,cb_handle));

  EXIT_DEBUG_MSG("sn_bp_remove_time_cb","result = %d",result);
}
...
static int sn_bp_get_num_children(const char * component_name) 
{
  ...
  result = (int) SN_DISPATCH(sn_ml_get_num_children,SN_UVM_ML_ADAPTER,sn_ML_LIB_adapter,(SN_UVM_ML_ADAPTER,component_name));
...
{ 
  ...
  result = (SN_TYPE(string)) SN_DISPATCH(sn_ml_get_child_name,SN_UVM_ML_ADAPTER,sn_ML_LIB_adapter,(SN_UVM_ML_ADAPTER,component_name,num));
...
{ 
  ...
  result = (SN_TYPE(string)) SN_DISPATCH(sn_ml_get_component_type_name,SN_UVM_ML_ADAPTER,sn_ML_LIB_adapter,(SN_UVM_ML_ADAPTER,component_name));
...
SN_TYPE(bool) sn_bp_is_port(const char * component_name) 
{
  ...
  result = (int) SN_DISPATCH(sn_ml_is_port,SN_UVM_ML_ADAPTER,sn_ML_LIB_adapter,(SN_UVM_ML_ADAPTER,component_name));
  EXIT_DEBUG_MSG("sn_bp_is_port","result = %d",result);
  return result;
}

static void sn_bp_print_srv_print_request(const char * message) {
  if (!SN_UVM_ML_ADAPTER) {
    CREATE_SN_UVM_ML_ADAPTER_IF_NEEDED;
  }
  SN_DISPATCH(sn_ml_print_srv_print_request,SN_UVM_ML_ADAPTER,sn_ML_LIB_adapter,(SN_UVM_ML_ADAPTER,message));
}
...
static int sn_bp_notify_command(const char * command,bp_output_cb_t * output_cb, uvm_ml_time_unit time_unit, double time_value) {
  ...
  if (SN_DISPATCH(sn_ml_check_ml_command_exists,SN_UVM_ML_ADAPTER,sn_ML_LIB_adapter,(SN_UVM_ML_ADAPTER,command))) {
...
SN_TYPE(bool) ml_sn_run_test(SN_LIST(string) tops,SN_TYPE(string) testname) {

  int result, tops_size, i;
  char ** top_names = NULL;

  ENTER_DEBUG_MSG_VOID("ml_sn_run_test");
  
  tops_size = SN_LIST_SIZE(tops);
 
  if (tops_size > 0) {
    top_names = malloc(tops_size * sizeof(char *));
						
    for (i=0;i<tops_size;i++) {
      top_names[i] = strdup(SN_LIST_GET(tops,i,string));
      if (debug_mode) {
	printf("%s ml_sn_run_test(): top[%d] = %s\n",ADAPTER_DBG_NAME,i,top_names[i]);
      }
    }
  }
  
  if (debug_mode) {
    printf("%s ml_sn_run_test(): testname = %s\n",ADAPTER_DBG_NAME,(testname) ? testname : "NULL");
  }
  
  result = provided_tray->run_test_ptr(sn_uvm_ml_adapter_id,tops_size,top_names,testname);
  ...
  EXIT_DEBUG_MSG("ml_sn_run_test","result = %d",result);

  return result;

}

void ml_sn_notify_time_cb(int target_frmw_id, uvm_ml_time_unit time_unit, SN_TYPE(real) time_value, SN_TYPE(external_pointer) cb_data) {

  double real_time_value = SN_REAL_GET(time_value);

  ENTER_DEBUG_MSG("ml_sn_notify_time_cb","target_frmw_id = %d, time_unit = %d, time_value = %f, cb_data = %p",
		  target_frmw_id,time_unit,real_time_value,cb_data);

  provided_tray->notify_time_cb_ptr(sn_uvm_ml_adapter_id,target_frmw_id,time_unit,real_time_value,(void*)cb_data);

  EXIT_DEBUG_MSG_VOID("ml_sn_notify_time_cb");
}
...
static void fill_bp_required_tray() 
{
  ...
  required_tray.wakeup_srv_register_time_cb_ptr = sn_bp_register_time_cb;
  required_tray.wakeup_srv_remove_time_cb_ptr = sn_bp_remove_time_cb;
  ...
  required_tray.print_srv_print_request_ptr = sn_bp_print_srv_print_request;
