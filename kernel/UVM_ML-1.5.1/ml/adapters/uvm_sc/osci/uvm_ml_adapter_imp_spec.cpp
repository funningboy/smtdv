...
(
  ...
  switch (message) {
    ...
    case UVM_ML_QUASI_STATIC_PREPARE_TO_SIMULATE_ERR:
      SC_REPORT_ERROR("QUASI_STATIC_PREPARE_TO_SIMULATE","\n");
      CURRENT_SIMCONTEXT_SET_ERROR();
      break;
...
void uvm_ml_tlm_transrec_base::object(sc_object* b) {
  ...
  if (is_transmitter()) {
    ...
    while (obj) {
      uvm_ml_special_component * tmp = DCAST<uvm_ml_special_component * >(obj);
      //      if (typeid(*obj) == typeid(parent_component_proxy)) return;
      if (tmp) return; // Parent proxy shall not be registered as a root node
