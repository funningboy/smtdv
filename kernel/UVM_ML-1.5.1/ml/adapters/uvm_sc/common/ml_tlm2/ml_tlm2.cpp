...
*/
  ...
  public:
    static int extensions_size(const tlm_generic_payload * gp)
    {
...
const char* ml_tlm2_phase_to_string(tlm_phase phase) {
  ...
  default:                              // Fixed typo for default
...
void ml_tlm2_transaction_mapping_update_error(const string &socket_name, 
					       ...
					       tlm_phase phase) {
  string err_str = "ML-TLM2 non-blocking transaction did not follow the rules of the TLM2 standard.\n'" +  
...
void ml_tlm2_transaction_mapping_mm_error(const string &socket_name) {
  string err_str = "ML-TLM2 non-blocking transaction does not have a memory manager, and therefore cannot be mapped.\n Socket '" +  
...
uvm_ml_packer & operator << (uvm_ml_packer &p, const tlm_generic_payload & tlm_gp) { 
  uvm_ml_class_companion * companion = uvm_ml_class_company::get_companion(typeid(tlm_gp)); 
  ...
  if(companion == NULL) {
    UVM_ML_SC_REPORT_ERROR("UVM-SC adapter: packing failed due to unidentified type","");
      ...
      p.pack_obj_header(companion->get_type_name());
      ml_tlm2_gp_companion::pack_tlm_generic_payload(p, &tlm_gp);	
      companion->uvm_ml_automation(&p, (tlm_generic_payload *) &tlm_gp, NULL, ML_TLM2_PACK_OP); 
  }
...
void ml_tlm2_update_data_from_response(tlm_generic_payload &trans, 
  ...
  if (length > 0) {
    if (trans.get_data_ptr() == NULL) {
      string str = full_socket_name + ": " + "data pointer in the request transaction is NULL but data_length is not 0";
      SC_REPORT_FATAL("Error", str.c_str());
    }
    if (response_gp.get_data_ptr())
...
{  
  ...
  // update extensions   
  for (int j = 0; j < uvm_gp_wrapper_base::extensions_size(tlm_gp); j++) {
...
void ml_tlm2_gp_companion::pack_tlm_generic_payload(uvm_ml_packer& p, const tlm_generic_payload * tlm_gp) 
{
    tlm_extension_base * i_extension;
    unsigned int         i_extension_num = 0;
  
    if(tlm_gp == NULL) {
        SC_REPORT_FATAL("Error", "pack_tlm_generic_payload called with tlm_gp NULL");
        return;
    }

    p << tlm_gp->get_address() << tlm_gp->get_command();

    unsigned int len = tlm_gp->get_data_length();
    unsigned char * data_ptr = tlm_gp->get_data_ptr();
    if ((data_ptr == NULL) && (len > 0)) {
        SC_REPORT_FATAL("Error", "inconsistent tlm_generic_payload transaction: data pointer is NULL and data length is not 0");
        return;
    }
    ...
    p << len << tlm_gp->get_response_status();
...
void ml_tlm2_analysis_export_connector_base::write_bitstream(unsigned stream_size, void * stream) {
