...
void         ml_tlm2_pack(const tlm_generic_payload & val, uvm_ml_packed_obj* p);
...
class ml_tlm2_connector_base: public sc_module
{
  public :
    ...
    ml_tlm2_connector_base(const string & sname) { uvm_ml_utils::add_tlm2_connector(sname, this); }
    virtual ~ml_tlm2_connector_base() {}
    void set_intf_name(const string &s) { m_intf_name = s; }
    const string & get_intf_name() const { return m_intf_name; }
...
{
  public :
  ml_tlm2_target_connector_base(const string &sname) : ml_tlm2_connector_base(sname) {}
  virtual ~ml_tlm2_target_connector_base() {}
...
{
  public :
  ml_tlm2_initiator_connector_base(const string & sname): ml_tlm2_connector_base(sname) {}
  virtual ~ml_tlm2_initiator_connector_base() {}
...
class ml_tlm2_analysis_export_connector_base: public ml_tlm2_connector_base
{
  public:
  ml_tlm2_analysis_export_connector_base(const string & sname): ml_tlm2_connector_base(sname) {}
  virtual ~ml_tlm2_analysis_export_connector_base() {}
  void write_bitstream(unsigned stream_size, void * stream);
  virtual void write(uvm_ml_packed_obj& vt) = 0; 
};
...
class ml_tlm2_target_connector: public ml_tlm2_target_connector_base
  ,virtual public tlm::tlm_bw_transport_if<TYPES>
  {
    ...
    ml_tlm2_target_connector_base(sname),
    isocket("isocket"),
...
class ml_tlm2_initiator_connector: public ml_tlm2_initiator_connector_base
  ,virtual public tlm::tlm_fw_transport_if<TYPES>
  {
    ...
    ml_tlm2_initiator_connector_base(sname),
    tsocket("tsocket"),
    disable_transaction_mapping(false),
    disable_transaction_mapping_set(false),
    m_socket_name(sname)
...
template <class T = tlm::tlm_generic_payload>
    class ml_tlm2_analysis_port_connector: public ml_tlm2_connector_base, 
                                           virtual public tlm::tlm_analysis_if<T>
{
public:
    sc_export<tlm_analysis_if<T> > connector_export;
    std::string                    m_port_name;

    ml_tlm2_analysis_port_connector(sc_module_name name_, tlm_analysis_port<T> & p, string sname):
        ml_tlm2_connector_base(sname),
        connector_export("connector_export"),
        m_port_name(sname)
  {
      connector_export(*this);
      p.bind(connector_export);
      is_target = false;
      set_intf_name("tlm_analysis_if");
  }
  ...
  virtual void write( const T &t ) { 
      ...
      ml_tlm2_pack(t, &v);
      uvm_ml_tlm_trans::write(conn_id, &v);
  }
};

template <class T = tlm::tlm_generic_payload>
    class ml_tlm2_analysis_export_connector: public ml_tlm2_analysis_export_connector_base
  {
public:
    tlm_analysis_port<T> connector_port;
    std::string          m_port_name;

    ml_tlm2_analysis_export_connector(sc_module_name name_, sc_core::sc_export<tlm::tlm_analysis_if<T> > & p, string sname):
        ml_tlm2_analysis_export_connector_base(sname),
        connector_port("connector_port"),
        m_port_name(sname)
    {
      connector_port.bind(p);
      is_target = true;
      set_intf_name("tlm_analysis_if");
    }

  virtual void write( uvm_ml_packed_obj& vt ) { 
      T * tlm_gp = 0;
      tlm_generic_payload * t = 0;

      ml_tlm2_unpack_create(&vt, t);
      if (t) {
	  tlm_gp = dynamic_cast<T *>(t);
          connector_port.write(*tlm_gp);
      }
      else
	UVM_ML_SC_REPORT_ERROR("UVM-SC adapter: failed to unpack an incoming transaction to a declared transaction type of analysis_export ",""); 
  }
