...
public:
    /*! Constructor
      ...
      \return N/A
    */
  uvm_ml_tlm_conn_info(const string & name, unsigned id);
  /*! The name() property
    \return std::string
  */
...
public:
  ...
  virtual const string & get_intf_name() const;
  virtual const string & get_REQ_name() const;
  virtual const string & get_RSP_name() const;
  virtual void set_intf_name(const string & s); 
  virtual void set_REQ_name(const string & s); 
  virtual void set_RSP_name(const string & s); 
...
public:
  static const string & get_systemc_separator();
  static void register_static_top(const string & full_name);
  static int get_max_bits();
  static int get_max_words();
  static unsigned initialize_adapter();
  static unsigned FrameworkId();
  static unsigned uvm_ml_tlm_id_from_name(const string & name);
  ...
  static unsigned do_connect (const string & port_name, 
                              const string & export_name);

  // for delayed uvm_ml_connect calls
  static void add_pending_uvm_ml_connect_call(const string & port_name,
                                              const string & export_name,
  ...
  static unsigned get_type_id(const string & name);
  static char* get_type_name(unsigned id);

  static unsigned int assign_transaction_id();
  static void tlm2_turn_off_transaction_mapping(const string & port_name, 
                                                const string & export_name);
  ...
  static uvm_ml_tlm_conn_info* get_or_create_conn_info(const string & name);
  static uvm_ml_tlm_conn_info* get_conn_info(const string & name); 
  ...
  static void add_tlm2_connector(const string & s, ml_tlm2_connector_base* conn);
  static unsigned find_socket_id(const string & s);
  ...
  static int                   implement_create_child_junction_node(const string & component_type_name,
                                                                    const string & instance_name,
                                                                    const string & parent_full_name,
                                                                    int            parent_framework_id,
                                                                    int            parent_junction_node_id);
  static void                  set_trace_register(bool turn_on) { m_trace_register_on = turn_on; }
  static bool                  is_trace_register_on() { return m_trace_register_on; }
  static void                  trace_register_tlm(const string & tlm_kind, const char * port_name) { 
    if (is_trace_register_on()) {
      string msg = string("UVM-ML SystemC: ") + tlm_kind + " " + port_name + " registered for ML connection";
...
protected:
  ...
  static map<unsigned,uvm_ml_tlm_conn_info*>* conn_info_by_id;
  static map<sc_object*, uvm_ml_tlm_transrec_base*>* obj_transrec_map;
  static map<string, unsigned>* socket_map;
  static map<unsigned, ml_tlm2_connector_base*>* connector_map;
  static unsigned m_nextid;

  static bool m_trace_register_on;
...
#ifdef NC_SYSTEMC
    : uvm_ml_tlm_transmitter_base(name_), dummy_event("sync_"+counter++)
#else
    : uvm_ml_tlm_transmitter_base(name_), dummy_event()
#endif 
  {}
  ~uvm_ml_tlm_transmitter_tlm() { }
  virtual const string & interface_name() { static string s = "tlm"; return s; } // object->if_name()???
  unsigned connId;
  unsigned adapterId;
...
public:
  ...
  virtual const string & interface_name() { static const string intf_name("tlm"); return intf_name; } // object->if_name()???
  unsigned connId;
  unsigned adapterId;
  //
...
void uvm_ml_register_internal( \
  ...
  uvm_ml_utils::trace_register_tlm("TLM1 port", p->name());	\
} \
 \
template <typename T> \
void uvm_ml_register_internal( \
  ...
  if (uvm_ml::uvm_ml_utils::reset_detected()) return;     \
  string s = string(p->basename()) + "_rec"; \
  ...
  uvm_ml_utils::trace_register_tlm("TLM1 export", p->name()); \
}
...
void uvm_ml_register_internal( 
     sc_core::sc_port<tlm::tlm_analysis_if<T>,N,POL>* p,
     tlm_generic_payload * t_ptr 
) { 
  uvm_ml_tlm2_register_internal(p);
} 
...
extern void uvm_ml_set_create_component(const string & component_type_name, const string & instance_name, const string & parent_name, const string & target_framework_indicator);
extern void uvm_ml_get_create_component_settings(string & component_type_name, string & instance_name, string & parent_name, string & target_framework_indicator);
