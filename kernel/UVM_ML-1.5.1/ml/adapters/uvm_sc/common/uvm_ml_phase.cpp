...
bool uvm_ml_phase::first_runtime_call = false;
                                 ...
                                 uvm_ml_phase_action phase_action)
{
    ...
    if (strcmp(phase_name,"resolve_bindings") == 0)
       return 1;

    uvm_phase *pphase = pschedule->set_phase(phase_name, state);
    if (pphase == NULL)
        return 0;
    ...
    pphase->execute_nonruntime_phase(pmod);
                               ...
                               uvm_ml_phase_action phase_action)
{
    ...
    // Ensure 'end_of_simulation' is called after final phase completes
    if (result && 
       (strcmp(phase_name, "final") == 0)  &&
       (state == UVM_PHASE_ENDED) )
    {
        UVM_ML_QUASI_STATIC_END_OF_SIMULATION();
    }
                                       ...
                                       unsigned int *      participate)
{
    ...
    // Upon first time entry to valid runtime notification, prepare to simulate
    if  (first_runtime_call == false)
    {
        first_runtime_call = true;
        try
        {
            QUASI_STATIC_PREPARE_TO_SIMULATE();
        }
#ifndef UVM_ML_PORTABLE
        UVM_ML_CATCH_SC_KERNEL_EXCEPTION_1(UVM_ML_QUASI_STATIC_PREPARE_TO_SIMULATE_ERR,0,0,0)
#else
        catch(int)
        {
            return 0;
