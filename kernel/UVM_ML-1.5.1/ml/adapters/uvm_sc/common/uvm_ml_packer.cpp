...
void ml_uvm_packer_rep_int::set_bits(unsigned* bits, unsigned nbits) {
  ...
  unsigned nwords = ((nbits-1)/32)+1; // GL unsigned nwords = nbits/32;
...
bool ml_uvm_packer_rep_int::check_obj_header() {
  bool result;
  unsigned unpack_index_sav = unpack_index;
  ...
  if (!unpack_non_null()) {
    ...
    std::string s = uvm_get_type_name(id);

    result = (((uvm_factory::is_uvm_object_registered(s)) || (uvm_factory::is_class_registered(s))) ? true : false);
  }
  unpack_index = unpack_index_sav;
  return result;
}
...
bool uvm_ml_packer_int::check_obj_header() {
  return m_rep_int->check_obj_header();
}
