...
public: \
  ...
  virtual void uvm_ml_automation(uvm_ml_packer * p, void * actual_arg, void* other_arg, ml_tlm2_oper op) { \
    T * actual UVM_UNUSED_ATTR = (T*) actual_arg; \
    T * other UVM_UNUSED_ATTR = (T*) other_arg;

#define ML_TLM2_GP_END(T) \
  } \
}; \
static bool ml_tlm2_##T##_dummy UVM_UNUSED_ATTR = uvm_ml_class_company::register_class_companion(typeid(T), new ml_tlm2_##T##_companion());
...
#define ML_TLM2_FIELD_ENUM_ACCESSORS(T, getr, setr) \
  if (op == ML_TLM2_PACK_OP) \
    (*p) << (int) actual->getr(); \
  else if (op == ML_TLM2_UNPACK_OP) { \
    int tmp; \
