...
public:
			 ...
			 , cons("consumer")
  {
    ...
    #ifdef MTI_SYSTEMC
    svtop = new child_component_proxy("testbench1");
    #else
    #endif
  }
  void before_end_of_elaboration() {
    ...
    void phase_started(uvm_phase* phase) { 
        ...
        if (phase->get_name() == "build") { 
            bool enable_prod = true;
            int  enable_con  = 1;
            short int sint   = 0x7FFF;
            long long int lint   = 0x7FFFFFFFFFFFFFFFLL;

            unsigned int       uintval = 0xFFFFFFFF;
            unsigned short int usint   = 0xFFFF;
            unsigned long long  int ulint   = 0xFFFFFFFFFFFFFFFFULL;
            char      charval = 0xFF;
            sc_bv<96> bv;

            for (int i = 0; i < 3; i++)
                {
                    bv.set_word(i, 0xFFFFFFFF);
                }

            string config_string = "SC Config string test value";

            packet *config_packet = new packet;
            config_packet->data = 505;

            cout << "SC env::build " << this->name() << endl;
            set_config_int("*env", "enable_prod", enable_prod);
            set_config_int("*env", "enable_con", enable_con);
            ...
            set_config_int("*env", "bit96", bv);
            set_config_int("*env", "char", charval);
            set_config_string("*env", "config_string", config_string);
            set_config_object("*env", "config_object", config_packet);
#ifdef MTI_SYSTEMC
            assert (uvm_ml_create_component("SV","testbench","testbench1",this) == svtop);
#else
            svtop = uvm_ml_create_component("SV", "testbench", "testbench1", this);
  ...
  void build() { 
    cout << " SC env - BUILD" << endl;
    cout << "SC ENV : FINISHING BUILD" << endl;
  }
  void connect() { 
...
public:
  ...
  void connect() { 
    ...
    uvm_ml_connect("sc_main/sctop/sc_env.testbench1.sv_env.producer.aport", sc_env.cons.aexport.name());
    uvm_ml_connect(sc_env.prod.aport.name(), "sc_main/sctop/sc_env.testbench1.sv_env.consumer.aimp");
#else
    uvm_ml_connect("sctop.sc_env.testbench1.sv_env.producer.aport", sc_env.cons.aexport.name());
    uvm_ml_connect(sc_env.prod.aport.name(), "sctop.sc_env.testbench1.sv_env.consumer.aimp");
#endif
  }
  void start_of_simulation() { 
