...
void uvm_component::end_of_elaboration_phase(uvm_phase* phase) {
   bool error;
   elaboration_done(error);     // TODO 'error' not initalized here?
  //end_of_elaboration();
}
...
// TODO - 'reset()' conflicts with component spec (see OSCI sub-directory)
//  phase->barrier.raise_barrier();
//  reset();
//  phase->barrier.drop_barrier();
}
