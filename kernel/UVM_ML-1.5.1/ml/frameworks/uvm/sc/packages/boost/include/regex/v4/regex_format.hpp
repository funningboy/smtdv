...
#define UVMSC_BOOST_REGEX_FORMAT_HPP
...
#include <packages/boost/include/type_traits/remove_pointer.hpp>
#include <packages/boost/include/type_traits/remove_cv.hpp>
#include <packages/boost/include/mpl/if.hpp>
#include <packages/boost/include/mpl/and.hpp>
#include <packages/boost/include/mpl/not.hpp>
#ifndef UVMSC_BOOST_NO_SFINAE
#include <packages/boost/include/mpl/has_xxx.hpp>
#endif
#include <packages/boost/include/ref.hpp>
...
// Forward declaration:
//
   template <class BidiIterator, class Allocator = UVMSC_BOOST_DEDUCED_TYPENAME std::vector<sub_match<BidiIterator> >::allocator_type >
class match_results;
...
private:
   ...
   {
      typedef typename uvmsc_boost::is_convertible<ForwardIter, const char_type*>::type tag_type;
...
#endif
   inline int toi(ForwardIter& i, ForwardIter j, int base, const uvmsc_boost::mpl::false_&)
   {
      if(i != j)
      {
         std::vector<char_type> v(i, j);
...
#pragma warning(pop)
#endif
   inline int toi(ForwardIter& i, ForwardIter j, int base, const uvmsc_boost::mpl::true_&)
   {
...
void basic_regex_formatter<OutputIterator, Results, traits, ForwardIter>::format_all()
{
   ...
   while(m_position != m_end)
   {
      ...
      {
      case '&':
         if(m_flags & ::uvmsc_boost::regex_constants::format_sed)
         {
            ++m_position;
            put(m_results[0]);
            break;
      ...
      case '\\':
         ...
         break;
      case '(':
         if(m_flags & uvmsc_boost::regex_constants::format_all)
         {
            ++m_position;
            ...
            if(m_position == m_end)
               return;
            UVMSC_BOOST_ASSERT(*m_position == static_cast<char_type>(')'));
      ...
      case ')':
         ...
         break;
      case ':':
         if((m_flags & uvmsc_boost::regex_constants::format_all) && m_have_conditional)
         {
            return;
         }
         put(*m_position);
...
void basic_regex_formatter<OutputIterator, Results, traits, ForwardIter>::format_perl()
{
   ...
   default:
      ...
      {
         std::ptrdiff_t len = ::uvmsc_boost::re_detail::distance(m_position, m_end);
...
void basic_regex_formatter<OutputIterator, Results, traits, ForwardIter>::format_escape()
{
   ...
   default:
      ...
      if((m_flags & uvmsc_boost::regex_constants::format_sed) == 0)
      {
         bool breakout = false;
         switch(*m_position)
         {
      ...
      int v = this->toi(m_position, m_position+len, 10);
      if((v > 0) || ((v == 0) && (m_flags & ::uvmsc_boost::regex_constants::format_sed)))
      {
         put(m_results[v]);
         break;
      }
      else if(v == 0)
      {
         ...
         --m_position;
         len = ::uvmsc_boost::re_detail::distance(m_position, m_end);
         len = (std::min)(static_cast<std::ptrdiff_t>(4), len);
         v = this->toi(m_position, m_position + len, 8);
         UVMSC_BOOST_ASSERT(v >= 0);
         put(static_cast<char_type>(v));
         break;
      }
...
#ifndef UVMSC_BOOST_NO_STD_ITERATOR
...
#ifdef UVMSC_BOOST_NO_STD_ITERATOR
...
UVMSC_BOOST_MPL_HAS_XXX_TRAIT_DEF(const_iterator)
...
template<typename Formatter, bool IsFunction = uvmsc_boost::is_function<Formatter>::value>
...
private:
   ...
   //
   UVMSC_BOOST_STATIC_ASSERT((::uvmsc_boost::is_pointer<F>::value || ::uvmsc_boost::is_function<F>::value || ::uvmsc_boost::is_class<F>::value));
   static formatter_wrapper<typename unwrap_reference<F>::type> f;
   static M m;
   static O out;
   static uvmsc_boost::regex_constants::match_flag_type flags;
public:
   UVMSC_BOOST_STATIC_CONSTANT(int, value = sizeof(check_is_formatter(f(m), f(m, out), f(m, out, flags))));
};
...
public:
   ...
   //
   typedef typename uvmsc_boost::mpl::if_<
      uvmsc_boost::mpl::and_<uvmsc_boost::is_pointer<F>, uvmsc_boost::mpl::not_<uvmsc_boost::is_function<typename uvmsc_boost::remove_pointer<F>::type> > >,
      uvmsc_boost::mpl::int_<0>,
      typename uvmsc_boost::mpl::if_<
         has_const_iterator<F>,
         uvmsc_boost::mpl::int_<1>,
         uvmsc_boost::mpl::int_<format_traits_imp<F, M, O>::value>
      >::type
   >::type type;
   //
   ...
   //
   UVMSC_BOOST_STATIC_ASSERT( uvmsc_boost::is_class<F>::value && !has_const_iterator<F>::value ? (type::value > 1) : true);
};

#else // UVMSC_BOOST_NO_SFINAE
...
public:
   ...
   //
   typedef typename uvmsc_boost::mpl::if_<
      uvmsc_boost::is_pointer<F>,
      uvmsc_boost::mpl::int_<0>,
      uvmsc_boost::mpl::int_<1>
   >::type type;
};

#endif // UVMSC_BOOST_NO_SFINAE
...
struct format_functor3
{
   ...
   OutputIter operator()(const Match& m, OutputIter i, uvmsc_boost::regex_constants::match_flag_type f)
   {
      return uvmsc_boost::unwrap_ref(func)(m, i, f);
   }
   template <class OutputIter, class Traits>
   OutputIter operator()(const Match& m, OutputIter i, uvmsc_boost::regex_constants::match_flag_type f, const Traits&)
   {
      return (*this)(m, i, f);
   }
private:
...
struct format_functor2
{
   ...
   OutputIter operator()(const Match& m, OutputIter i, uvmsc_boost::regex_constants::match_flag_type /*f*/)
   {
      return uvmsc_boost::unwrap_ref(func)(m, i);
   }
   template <class OutputIter, class Traits>
   OutputIter operator()(const Match& m, OutputIter i, uvmsc_boost::regex_constants::match_flag_type f, const Traits&)
   {
...
struct format_functor1
{
   ...
   OutputIter operator()(const Match& m, OutputIter i, uvmsc_boost::regex_constants::match_flag_type /*f*/)
   {
      return do_format_string(uvmsc_boost::unwrap_ref(func)(m), i);
   }
   template <class OutputIter, class Traits>
   OutputIter operator()(const Match& m, OutputIter i, uvmsc_boost::regex_constants::match_flag_type f, const Traits&)
   {
...
struct format_functor_c_string
{
   ...
   template <class OutputIter>
   OutputIter operator()(const Match& m, OutputIter i, uvmsc_boost::regex_constants::match_flag_type f, const Traits& t = Traits())
   {
...
struct compute_functor_type
{
   ...
   typedef typename uvmsc_boost::remove_cv< typename uvmsc_boost::remove_pointer<Func>::type>::type maybe_char_type;

   typedef typename mpl::if_<
      ::uvmsc_boost::is_same<tag, mpl::int_<0> >, format_functor_c_string<maybe_char_type, Match, Traits>,
      typename mpl::if_<
         ::uvmsc_boost::is_same<tag, mpl::int_<1> >, format_functor_container<Func, Match, Traits>,
         typename mpl::if_<
            ::uvmsc_boost::is_same<tag, mpl::int_<2> >, format_functor1<Func, Match>,
            typename mpl::if_<
               ::uvmsc_boost::is_same<tag, mpl::int_<3> >, format_functor2<Func, Match>, 
...
#endif  // UVMSC_BOOST_REGEX_FORMAT_HPP
