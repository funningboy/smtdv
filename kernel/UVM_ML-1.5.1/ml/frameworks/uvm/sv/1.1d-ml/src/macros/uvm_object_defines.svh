  ...
  begin \
    case (what__) \
      ...
      UVM_UNPACK: \
        if(!((FLAG)&UVM_NOPACK)) begin \
          if(((FLAG)&UVM_NOPACK) == 0 && ((FLAG)&UVM_REFERENCE) == 0) \
            /* ML Additions (START) __m_uvm_status_container.packer.unpack_object(ARG); */ \
            begin \
              uvm_object lhs__; \
              lhs__ = ARG; \
...
`define uvm_field_sarray_object(ARG,FLAG) \
  begin \
    case (what__) \
      ...
      UVM_UNPACK: \
        if(!((FLAG)&UVM_NOPACK)) begin \
/* ML Additions (START)          foreach(ARG[i]) */ \
    ...
    case (what__) \
      ...
      UVM_PACK: \
        if(!((FLAG)&UVM_NOPACK)) begin \
          ...
          foreach(ARG[i])  \
/* ML Additions (START)              if($bits(ARG[i]) <= 64) __m_uvm_status_container.packer.pack_field_int(ARG[i], $bits(ARG[i])); */ \
            if($bits(ARG[i]) <= 64) __m_uvm_status_container.packer.pack_field_element(ARG[i], $bits(ARG[i])); \
/* ML Additions (END) */ \
            else __m_uvm_status_container.packer.pack_field(ARG[i], $bits(ARG[i])); \
        end \
      UVM_UNPACK: \
        if(!((FLAG)&UVM_NOPACK)) begin \
          ...
          foreach(ARG[i]) \
/* ML Additions (START)            if($bits(ARG[i]) <= 64) ARG[i] = __m_uvm_status_container.packer.unpack_field_int($bits(ARG[i])); */ \
            if($bits(ARG[i]) <= 64) ARG[i] = __m_uvm_status_container.packer.unpack_field_element($bits(ARG[i])); \
/* ML Additions (END) */ \
            else ARG[i] = __m_uvm_status_container.packer.unpack_field($bits(ARG[i])); \
        end \
      UVM_RECORD: \
    ...
    case (what__) \
      ...
      UVM_PACK: \
        if(!((FLAG)&UVM_NOPACK)) begin \
          /**/if(__m_uvm_status_container.packer.use_metadata) __m_uvm_status_container.packer.pack_field_int(ARG.size(), 32); \
          foreach(ARG[i])  \
/* ML Additions (START)            __m_uvm_status_container.packer.pack_field_int(int'(ARG[i]), $bits(ARG[i])); */ \
            __m_uvm_status_container.packer.pack_field_element(int'(ARG[i]), $bits(ARG[i])); \
/* ML Additions (END) */ \
        end \
      UVM_UNPACK: \
        if(!((FLAG)&UVM_NOPACK)) begin \
          ...
          /**/end \
          foreach(ARG[i]) \
/* ML Additions (START)             ARG[i] = T'(__m_uvm_status_container.packer.unpack_field_int($bits(ARG[i]))); */ \
            ARG[i] = T'(__m_uvm_status_container.packer.unpack_field_element($bits(ARG[i]))); \
/* ML Additions (END) */ \
        end \
      UVM_RECORD: \
