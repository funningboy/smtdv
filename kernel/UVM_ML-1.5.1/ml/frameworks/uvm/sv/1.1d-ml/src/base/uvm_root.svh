...
//----------------------
  ...
  extern static function uvm_component add_top_level (string top_name, string instance_name);
  extern function bit                  do_nonblocking_phase (string phase_name);
  extern function void                 do_blocking_phase (string phase_name);
  extern static function void          ignore_run_test();

  extern function void end_test();

  bit m_end_test_done = 0;
  static bit m_ignore_run_test = 0;
...
//----------------------
  ...
  if (m_ignore_run_test == 1) begin
    uvm_report_warning("RNTST",
          "run_test() is not used in this flow, this invocation will be a no-op");
    return;
  end
//--------------------
// ML Additions (END)
...
function uvm_component uvm_root::add_top_level (string top_name, string instance_name);

  uvm_component this_top;

  $cast(this_top, factory.create_component_by_name (top_name, "", instance_name, null));
  return this_top;
endfunction

function bit uvm_root::do_nonblocking_phase (string phase_name);
  uvm_phase phase;
  uvm_domain domain;
  ...
  if (m_phase_all_done == 1)
    end_test();
endfunction

function void uvm_root::do_blocking_phase (string phase_name);
  string test_name = "";
...
function void uvm_root::end_test();
...
function void uvm_root::ignore_run_test();
  m_ignore_run_test = 1;
endfunction
