...
class uvm_phase extends uvm_object;
  ...
  extern protected task  execute_phase(uvm_component comp = null, bit bsequential = 0);    // ML - Modification
...
// ML Additions (START)
//----------------------
extern function void ml_execute_phase(uvm_component top, uvm_phase_state state = UVM_PHASE_STARTED);


extern static protected task ml_hopper_peek(output uvm_phase phase);
extern static protected task ml_hopper_get(output uvm_phase phase);
extern static protected task ml_hopper_put(uvm_phase phase);
extern static protected function bit ml_hopper_try_peek(output uvm_phase phase);
extern static protected function bit ml_hopper_try_get(output uvm_phase phase);
extern static protected function bit ml_hopper_try_put(uvm_phase phase);
...
task uvm_phase::execute_phase(uvm_component comp = null, bit bsequential = 0); // ML - Modification

  uvm_component  traverse_top;  // ML Modification
...
//----------------------
  ...
  if (m_phase_type != UVM_PHASE_NODE) begin
    ...
    if (!bsequential)  // ML - Modification
      #0;
    ...
    //---------
    m_state = UVM_PHASE_STARTED;
    m_imp.traverse(traverse_top,this,UVM_PHASE_STARTED);    // ML - Modifications
    ...
    if (!$cast(task_phase,m_imp)) begin
      ...
      m_state = UVM_PHASE_EXECUTING;
       if (!bsequential)  // ML - Modification
	 #0; // LET ANY WAITERS WAKE UP
      m_imp.traverse(traverse_top,this,UVM_PHASE_EXECUTING);    // ML - Modifications
           ...
           fork
             ...
             begin
               ...
               while (do_ready_to_end) begin
                 ...
                 if (m_imp != null)
                 begin
                   m_imp.traverse(traverse_top,this,UVM_PHASE_READY_TO_END);    // ML - Modifications
                 end
  ...
  if(m_jump_fwd || m_jump_bkwd) begin
    ...
    if (m_imp != null)
    begin
       m_imp.traverse(traverse_top,this,UVM_PHASE_ENDED);   // ML - Modifications
    end
    if (!bsequential)  // ML - Modification
      #0; // LET ANY WAITERS WAKE UP
  ...
  if ((task_phase == null) && (!bsequential))  // ML - Modification
    m_wait_for_pred();
...
function void uvm_phase::ml_execute_phase(uvm_component top, uvm_phase_state state = UVM_PHASE_STARTED);
  ...
  begin
      if ((m_run_count == 0) && (m_phase_trace))
      begin
          `UVM_PH_TRACE("PH/TRC/STRT","Starting phase",this,UVM_LOW)
      end 
  ...
  end    // if state == UVM_PHASE_EXECUTING
  
  if (state == UVM_PHASE_READY_TO_END) begin
      `UVM_PH_TRACE("PH_READY_TO_END","PHASE READY TO END",this,UVM_DEBUG)
      if (m_phase_trace)
          `UVM_PH_TRACE("PH_READY_TO_END_CB","CALLING READY_TO_END CB",this,UVM_HIGH)
      m_state = UVM_PHASE_READY_TO_END;
      if (m_imp != null)
      begin
          m_imp.traverse(top,this,UVM_PHASE_READY_TO_END);    // ML Modification
      end
  end
  ...
  //-------
  // execute 'phase_ended' callbacks
  if (state == UVM_PHASE_ENDED)
  begin
      m_executing_phases.delete(this);
      if (m_run_count == 1) begin
        if (m_phase_trace)
          `UVM_PH_TRACE("PH_END","ENDING PHASE",this,UVM_HIGH)
        m_state = UVM_PHASE_ENDED;
      end
      ...
      // DONE:
      //------
      if ( m_run_count == uvm_root::get_num_top_levels()) begin
        if (m_phase_trace)
          `UVM_PH_TRACE("PH/TRC/DONE","Completed phase",this,UVM_LOW)
        m_state = UVM_PHASE_DONE;
      end
...
task uvm_phase::ml_hopper_peek(output uvm_phase phase);
    m_phase_hopper.peek(phase);
endtask

task uvm_phase::ml_hopper_get(output uvm_phase phase);
    m_phase_hopper.get(phase);
endtask

task uvm_phase::ml_hopper_put(uvm_phase phase);
    m_phase_hopper.put(phase);
endtask

function bit uvm_phase::ml_hopper_try_peek(output uvm_phase phase);
    return m_phase_hopper.try_peek(phase);
endfunction

function bit uvm_phase::ml_hopper_try_get(output uvm_phase phase);
    return m_phase_hopper.try_get(phase);
endfunction

function bit uvm_phase::ml_hopper_try_put(uvm_phase phase);
    return m_phase_hopper.try_put(phase);
endfunction
