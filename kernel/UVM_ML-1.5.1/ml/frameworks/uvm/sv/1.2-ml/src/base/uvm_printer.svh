...
function string uvm_tree_printer::emit();
  ...
  foreach (m_rows[i]) begin
    ...
    if (user_format == "") begin
      ...
      // Name (id)
      if (knobs.identifier) begin
        s.push_back({indent_str, row.name});
      ...
      if (row.val[0] == "@") // is an object w/ knobs.reference on
        s.push_back({"(",row.type_name,row.val,") "});
      else
        if (knobs.type_name &&
             (row.type_name != "" ||
              row.type_name != "-" ||
              row.type_name != "..."))
          s.push_back({"(",row.type_name,") "});
      ...
      if (i < m_rows.size()-1) begin
        if (m_rows[i+1].level > row.level) begin
          s.push_back({string'(knobs.separator[0]), linefeed});
          continue;
        end
      end

      // Value (unconditional)
      s.push_back({row.val, " ", linefeed});
      ...
      if (i <= m_rows.size()-1) begin
        ...
        if (end_level < row.level) begin
          ...
          for (int l=row.level-1; l >= end_level; l--) begin
            ...
            s.push_back({indent_str, string'(knobs.separator[1]), linefeed});
          end
        end
      end
