...
`include "macros/uvm_version.svh"
...
// `define UVM_NAME CDNS-UVM

`define UVM_NAME ML-OA-UVM
...
`ifdef UVM_FIX_REV
 `define UVM_VERSION_STRING `"`UVM_NAME``-```UVM_MAJOR_REV``.```UVM_MINOR_REV`UVM_FIX_REV (`UVM_KIT_VERSION)`"
`else
 `define UVM_VERSION_STRING `"`UVM_NAME``-```UVM_MAJOR_REV``.```UVM_MINOR_REV`` (`UVM_KIT_VERSION)`"
`endif
