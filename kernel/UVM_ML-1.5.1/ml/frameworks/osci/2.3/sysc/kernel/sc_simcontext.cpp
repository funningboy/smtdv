...
namespace uvm_ml_sc_changes {
    static bool sc_start_disabled = false;
    void disable_sc_start() { 
        sc_start_disabled = true;
    }
}
// DEBUGGING MACROS:
//
...
static bool in_cosim = false; // using static variables to avoid class modification

static bool in_quasi_static_stage = false;

// Helper variables
// We are trying to keep our changes to the OSCI library confined to sc_simcontext only
// so as to allow the users to reuse their custom-compiled versions of the library, 
// while we replace sc_simcontext.o only, on-the-fly
// Declaring all the "alternative registries" fields would have forced us to supply 
// and mandate a custom OSCI library version

namespace {
    static sc_module_registry* static_modules = NULL;
    static sc_port_registry* static_ports = NULL;
    static sc_export_registry* static_exports = NULL;
    static sc_prim_channel_registry* static_prim_channels = NULL;
    static bool registries_replaced = false;
}
...
/////////////////////
void sc_simcontext::quasi_static_start_of_construction() 
{
    ...
    if (m_elaboration_done) {
        ...
        m_elaboration_done = false;
        m_ready_to_simulate = false;
        in_quasi_static_stage = true;
        
        static_modules = m_module_registry;
        m_module_registry = new sc_module_registry(*this);
        
        static_ports = m_port_registry;
        m_port_registry = new sc_port_registry(*this);
        
        static_exports = m_export_registry;
        m_export_registry = new sc_export_registry(*this);
    
        static_prim_channels = m_prim_channel_registry;
        m_prim_channel_registry = new sc_prim_channel_registry(*this);
        
        registries_replaced = true;
    }
}

/////////////////////

bool sc_simcontext::check_stop()
{
  // check for call(s) to sc_stop
  if( m_forced_stop ) {
      do_sc_stop_action();
...
void sc_simcontext::quasi_static_end_of_construction() 
{
  m_in_simulator_control = true;
  m_port_registry->construction_done();
  m_export_registry->construction_done();
  m_prim_channel_registry->construction_done();
  //  NOT !!! m_module_registry->construction_done();
}

void sc_simcontext::quasi_static_complete_binding() {
    m_port_registry->complete_binding();
}

bool sc_simcontext::signal_end_of_elaboration() {
    // SIGNAL THAT ELABORATION IS DONE
    //
    // We set the switch before the calls in case someone creates a process 
    // in an end_of_elaboration callback. We need the information to flag 
...
void sc_simcontext::quasi_static_end_of_elaboration() {
    void(signal_end_of_elaboration());
}

bool sc_simcontext::signal_start_of_simulation() {
...
void sc_simcontext::quasi_static_start_of_simulation() 
{
    m_start_of_simulation_called = false;
    signal_start_of_simulation();
    // FIXME - check the status of signal_start_of_simulation
    // What to do if some entity requests to stop?

    if (registries_replaced)
    {
        // Merge the registries
        // Push newly added objects into the registries keeping the 'old' ones
        // while trying to keeps things in order of addition
        for (int i = 0; i < m_module_registry->size(); ++i)
        {
            static_modules->m_module_vec.push_back(m_module_registry->m_module_vec[i]);
        }

        delete m_module_registry; m_module_registry = static_modules;

        for (int i = 0; i < m_port_registry->size(); ++i)
        {
            static_ports->m_port_vec.push_back(m_port_registry->m_port_vec[i]);
        }
        
        delete m_port_registry; m_port_registry = static_ports;
        
        for (int i = 0; i < m_export_registry->size(); ++i)
        {
            static_exports->m_export_vec.push_back(m_export_registry->m_export_vec[i]);
        }

        delete m_export_registry; m_export_registry = static_exports;

        for (int i = 0; i < m_prim_channel_registry->size(); ++i)
        {
            static_prim_channels->m_prim_channel_vec.push_back(m_prim_channel_registry->m_prim_channel_vec[i]);
        }

        delete m_prim_channel_registry; m_prim_channel_registry = static_prim_channels;
    }

    in_quasi_static_stage = false;
}
...
void
sc_simcontext::do_simulate( const sc_time& duration )
{
...
void
sc_simcontext::co_simulate( const sc_time& duration )
{
  ...
  in_cosim = true;
	...
	do {
	    // See note 1 above:
            if (! next_time(t) ) { t = until_t; goto exit; }
            if  (t > until_t )  { t = until_t; goto exit; }
	    if ( t > m_curr_time ) 
	    {
	        m_curr_time = t;
		m_change_stamp++;
    ...
    } while ( t <= until_t ); // UVM_ML: co-simulate - do enter until_t // WAS: hold off on the delta for the until_t time ( t < until_t ) 
...
void sc_simcontext::quasi_static_prepare_to_simulate()
{
  prepare_to_simulate();
  initial_crunch(true);
  m_in_simulator_control = false;
  co_simulate(SC_ZERO_TIME);
}
...
void
sc_simcontext::quasi_static_module_construction_done(sc_module * node)
{
    node->construction_done();
}

void
sc_simcontext::quasi_static_module_end_of_elaboration(sc_module * node)
{
    bool err;
    node->elaboration_done(err);
}

void
sc_simcontext::quasi_static_module_start_of_simulation(sc_module * node)
{
...
/*****************************************************************************
  ...
  Description of Modification: - changed process existence structures to
      ...
      Name, Affiliation, Date: Vitaly Yankelevich, Gabi Leshem, Alex Chudnovsky, 
                               Cadence Design Systems 28 February 2013
  Description of Modification: - added UVM-ML extensions to enable multi language support
				 under UVM-ML
 *****************************************************************************/
